...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen,
                  gen_val, total_epoch, cuda, fp16, scaler, backbone, save_period, save_dir):
    total_r_loss = 0
    total_c_loss = 0
    total_loss = 0
    val_loss = 0

    # <editor-folder desc="训练">
    pbar = tqdm(total=epoch_step, desc=f'[train     ]\tEpoch {epoch + 1}/{total_epoch}', postfix=dict, ncols=150, mininterval=0.3)
    model_train.train()
    ...
    # <editor-folder desc="验证">
    pbar = tqdm(total=epoch_step_val, desc=f'[validation]\tEpoch {epoch + 1}/{total_epoch}', postfix=dict, ncols=100, mininterval=0.3)
    model_train.eval()
